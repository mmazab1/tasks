...
export function addNewQuestion(
    ...
    const newQuestion = Objects.makeBlankQuestion(id, name, type);
...
export function editOption(
    ...
    return questions.map((q) =>
        q.id === targetId ? { ...q, options: updateOptions(q.options) } : q,
    );
}
...
export function duplicateQuestionInArray(
    ...
    for (const q of questions) {
        result.push(q);
        if (q.id === targetId) {
            result.push(Objects.duplicateQuestion(newId, q));
        }
    }
